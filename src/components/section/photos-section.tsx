      ...
      <div className="flex min-h-0 flex-col gap-y-4">
        <div className="flex items-center w-full">
          ...
          <div className="border bg-primary z-10 rounded-xl px-4 py-1">
            <span className="text-background text-sm font-medium">项目界面</span>
          </div>
              ...
              {col.map((photo, idx) => (
                <BlurFade key={photo.src} delay={BLUR_FADE_DELAY * 14 + (colIdx * col.length + idx) * 0.05}>
                  <img
                    src={photo.src}
                    alt={photo.alt}
                    className={`w-full rounded-xl object-contain bg-muted/30 ${colAspects[colIdx][idx] ?? "aspect-square"}`}
                  />
                </BlurFade>
              ))}
            </div>
